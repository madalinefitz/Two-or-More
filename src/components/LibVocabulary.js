...
export default function LibVocabulary() {
    ...
    const wordComponent = vocab.map( word => {
        return(
            <div>
                {showBack ?
                    <div onClick={handleFlip} class="hover:cursor-pointer bg-[#E8E4DE] rounded-full shadow-lg backdrop-blur-lg dark:bg-slate-800/50 w-52 h-52 p-10">
                        <p class="text-[#292929] text-xl text-center font-semibold">{word.definition}</p>
                    </div> 
                    :
                    <div onClick={handleFlip} class="hover:cursor-pointer bg-[#292929] rounded-full shadow-lg backdrop-blur-lg dark:bg-slate-800/50 w-52 h-52 p-10">
